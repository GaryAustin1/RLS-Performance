...
Please the [last section](#tools-to-measure-performance) for ways to measure the performance of you queries as you test RLS improvements.
...
Instead of having this RLS where the roles_table has a RLS select policy of `auth.uid() = user_id`:  
`exists (select 1 from roles_table where auth.uid() = user_id and role = "good_role")`  
Create a security definer function get_role() (see link) and do:  
`has_role()`  
