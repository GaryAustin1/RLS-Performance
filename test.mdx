...
#### Another method to improve performance is to wrap your RLS queries and functions in select statements. 
...
#### Always optimize join queries to compare row columns to fixed join data.
...
| Test | RLS Before  |  RLS After   | SQL|  SQL | JS | JS |  
|-----|------------------------|---------------------------|------|------|---|---- |  
| 1 | auth.uid()=user_id |user_id indexed | 171ms| <.1|  731 | <.1 |  
| 2a| auth.uid()=user_id |(select auth.uid()) = user_id|179|9|735|10|  
|2b|is_admin() *table join* | (select is_admin()) *table join*| 11,000 | 7 | T OUT | 7| 
|3| auth.uid()=user_id| add .eq or where on user_id | 171 | 9 | 740 | 10 |  
